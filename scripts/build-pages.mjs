...
function housingImageCards(locale, badgeCodes = false) {
  ...
  const confirmedCodes = new Set(['FB', 'FL', 'P', 'T', 'F', 'FC', 'FU', 'PH', 'PA']);
  ...
  return housingGalleryImages.map(src => {
    ...
    const code = badgeCodes && confirmedCodes.has(series.seriesCode) ? `<h2 dir="ltr">${escapeHtml(series.seriesCode)}</h2>` : '';
