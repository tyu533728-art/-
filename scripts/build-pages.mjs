...
const displayItemAlts = {
  ...
  vi: { UCT: 'UCT vòng bi lắp có vòng trong rộng, khóa bằng vít chặn', UEL: 'UEL vòng bi lắp có vòng trong rộng, khóa bằng vòng lệch tâm', UK: 'UK vòng bi lắp có lỗ côn cho ống lót chặn', FB: 'FB gối đỡ chân đế', P: 'P gối đỡ chân đế', PA: 'PA gối đỡ chân đế có đế ren', F: 'F gối đỡ mặt bích bốn bu lông', FL: 'FL gối đỡ mặt bích hai bu lông', FC: 'FC gối đỡ mặt bích tròn có gờ định tâm', FS: 'FS gối đỡ mặt bích vuông có gờ định tâm', T: 'T gối đỡ tăng đơ' },
  ...
  ja: { UCT: 'UCT 広幅内輪付き止めねじ固定インサート軸受', UEL: 'UEL 広幅内輪付き偏心ロックカラー固定インサート軸受', UK: 'UK アダプタスリーブ用テーパ穴付きインサート軸受', FB: 'FB ピローブロックハウジング', P: 'P ピローブロックハウジング', PA: 'PA ねじ込みベース付きピローブロックハウジング', F: 'F 4ボルトフランジハウジング', FL: 'FL 2ボルトフランジハウジング', FC: 'FC センタリング付き丸形フランジハウジング', FS: 'FS センタリング突起付き角形フランジハウジング', T: 'T テークアップハウジング' },
  ko: { UCT: 'UCT 넓은 내륜 세트스크루 잠금 인서트 베어링', UEL: 'UEL 넓은 내륜 편심 잠금 칼라 인서트 베어링', UK: 'UK 어댑터 슬리브용 테이퍼 보어 인서트 베어링', FB: 'FB 필로우 블록 하우징', P: 'P 필로우 블록 하우징', PA: 'PA 나사식 베이스 필로우 블록 하우징', F: 'F 4볼트 플랜지 하우징', FL: 'FL 2볼트 플랜지 하우징', FC: 'FC 센터링 원형 플랜지 하우징', FS: 'FS 센터링 돌출부가 있는 사각 플랜지 하우징', T: 'T 테이크업 하우징' }
};
...
function housingImageCards(locale, badgeCodes = false) {
  ...
  const confirmedCodes = new Set(['FB', 'FL', 'P', 'T', 'F']);
